...
Remove single-letter words - `filter(str_count())`
`str_count()` = Count the number of characters in a string
...
1. Loading Google Fonts - `font_add_google()`
...
`[KoNLP](https://github.com/youngwoos/Doit_R/blob/master/FAQ/install_KoNLP.md)` Installing the Korean Morphological Analysis Package. The order of installation is important, so be sure to do it in that order.
