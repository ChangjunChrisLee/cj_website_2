...
Measurement error is the discrepancy between the observed value and the true value of a variable. [SEM explicitly incorporates measurement error into its models, which sets it apart from traditional regression approaches]{.underline}.
...
>
> SEM은 **모델을 어떻게 그리느냐에 따라 추정값이 달라지는 구조**이기 때문.
>
...
[Develop a theoretical model based on prior research or hypotheses.]{.underline} Clearly specify the relationships among variables in a path diagram. (탐색적 분석에는 부적합)
...
> 하나의 잠재 변수에 측정변수는 왜 3개 이상이 권장될까?
>
> **1. 잠재변수(latent variable)를 식별(identification)하기 위한 기본 기준**
>
> 잠재변수 1개를 CFA/SEM에서 추정하려면 **최소 2개의 지표(아이템)** 이면 모델을 **식별 가능**하게 만들 수 있음.
>
> 하지만 2개만 있을 때는 아래 문제가 생김.
>
> -   잠재변수의 **오차분산이 제대로 분리되지 않음**
>
> -   **모델 식별이 불안정**
>
> -   **추정값의 표준오차가 커짐**
>
> -   **신뢰도 검증(CR, AVE, Cronbach α)이 부정확**
>
> -   그래서 **실무와 학계에서는 3개 이상**을 추천하는 것.
>
> 그렇다면 왜 2개는 불안한가?
>
> **① 2개 지표는 사실상 “그래야만 겨우 식별”되는 구조라서** 구조적 제약을 강제로 걸어야 식별이 됨.
>
> -   예: 두 아이템의 loading 을 동일하게 설정하거나, 잠재변수 분산을 1로 고정하는 식. 즉, **자연스럽지 않은 형태로 모델을 고정해야 함.**
>
> **② 신뢰도 검증이 어려움**
>
> 2개만 있으면:
>
> -   Cronbach’s α 가 안정적이지 않음
>
> -   AVE 계산 시 분산 추정이 불안정
>
> -   Composite Reliability(CR)가 낮게 나오는 경향
>
> **③ 단일 아이템 편향(single-item bias)이 강하게 작동**
>
> 아이템이 적을수록
>
> -   각 문항의 측정 오류가 잠재변수 전체에 그대로 전달
>
> -   비정상적인 loading 값이 생기면 수정할 여지가 없음
>
> -   즉, **두 문항 중 하나만 문제여도 잠재변수 전체가 흔들림.**
...
> **CFA/SEM에서 가장 널리 쓰는 4대 핵심 Model Fit 지표**
>
> -    **CFI (Comparative Fit Index)**
>
>     -   *기준*: **≥ 0.90 (권장 ≥ 0.95)**
>
>     -   기준모델(null model)과 비교했을 때 얼마나 개선되었는지.
>
> -   **TLI (Tucker–Lewis Index, NNFI)**
>
>     -   *기준*: **≥ 0.90 (권장 ≥ 0.95)**
>
>     -   모형 복잡성(자유도)을 감안한 상대 적합도.
>
> -   **RMSEA (Root Mean Square Error of Approximation)**
>
>     -   *기준*: **≤ 0.08 (좋음 ≤ 0.06)**
>
>     -   모형이 “근사적으로 얼마나 잘 맞는가”를 나타내는 핵심 지표.
>
> -   **SRMR (Standardized Root Mean Square Residual)**
>
>     -   *기준*: **≤ 0.08**
>
>     -   관찰 공분산 vs 모델이 예측한 공분산의 평균 차이.
...
```{r}
# -------------------------------------------------
# 1. Load psych package
# -------------------------------------------------
library(psych)

# -------------------------------------------------
# 2. Determine the number of factors
#    - Parallel analysis (가장 추천되는 방법)
# -------------------------------------------------
fa.parallel(observed_data, 
            fa = "fa",         # factor analysis
            n.iter = 100,      # bootstrap iterations
            show.legend = TRUE,
            main = "Parallel Analysis for Factor Selection")

# -------------------------------------------------
# 3. Run EFA (Exploratory Factor Analysis)
#    - 여기서는 3개 요인 지정 (위에서 우리가 생성한 구조)
#    - rotation = "promax": oblique rotation (요인 상관 허용)
# -------------------------------------------------
efa_result <- fa(observed_data, 
                 nfactors = 3, 
                 fm = "ml",      # maximum likelihood (추천)
                 rotate = "promax")

# -------------------------------------------------
# 4. Print EFA result
# -------------------------------------------------
print(efa_result, digits = 3)

# -------------------------------------------------
# 5. Factor loadings table only
# -------------------------------------------------
loadings <- print(efa_result$loadings, cutoff = 0.30)

# -------------------------------------------------
# 6. Visualize factor structure (optional)
# -------------------------------------------------
fa.diagram(efa_result)

# -------------------------------------------------
# 7. Extract factor scores for further analysis 
# -------------------------------------------------
...
-   **EFA:** 요인 구조를 *탐색*하는 분석 (무엇이 어디에 적재될지 모를 때)

-   **CFA:** 요인 구조를 *검증*하는 분석 (이론 기반 가설을 테스트할 때)

**2. 아이템–요인 관계 설정**

-   **EFA:** 모든 문항이 모든 요인에 적재될 가능성을 열어둠

-   **CFA:** 어떤 문항이 어떤 요인에만 적재될지 연구자가 *사전에 지정*

-   **EFA:** cross-loading 허용

-   **CFA:** cross-loading 대부분 금지

**3. 회전(rotation)**

-   **EFA:** 필수 (varimax, promax 등)

-   **CFA:** 없음 (모델 자체가 구조를 고정하고 있기 때문에)

**4. 연구자의 역할**

-   **EFA:** 데이터가 말하게 하는 분석(데이터 기반 구조 탐색)

-   **CFA:** 이론이 구조를 정하는 분석(가설 기반 구조 설정)

**5. 적합도 평가(Fit indices)**

-   **EFA:** 전통적으로 크게 사용 X (요인 수 결정과 loading 패턴이 핵심)

-   **CFA:** CFI, TLI, RMSEA, SRMR 등 *fit index 필수*

**6. 분석 맥락(언제 사용?)**

-   **EFA:** 새로운 척도 개발, 미지의 구조 탐색, 초기 단계

-   **CFA:** 이미 개발된 척도의 구조 검증, SEM으로 이어지는 단계

**7. 모델 제약**

-   **EFA:** 제약 거의 없음 → 자유도가 높음

-   **CFA:** 경로와 loading을 엄격하게 제약 → 이론 검증 중심

**8. 학문적 흐름**

-   **표준 절차:** EFA → CFA → SEM

-   EFA로 구조를 찾고, CFA로 검증하고, SEM으로 인과관계를 분석

<br>

> **요인분석에서 ‘회전(rotation)’이란?**
>
> -   요인 부하(loadings) 구조를 더 해석하기 쉽도록, 요인의 좌표축을 돌려서 문항들이 특정 요인에 더 선명하게 적재되도록 만드는 과정.
>
> -   EFA를 처음 실행하면 문항들이 여러 요인에 비슷하게 적재되어 해석이 모호하거나 요인별 특징이 명확하지 않음. **요인의 축을 돌려서(rotate)** “어떤 문항이 어떤 요인에 가장 가깝다”를 **더 명확하게 만들어주는 과정.**
>
> **회전의 종류**
>
> -   **직교 회전(orthogonal)**
>
>     -   요인 간 상관 = 0 으로 가정 (예: Varimax)
>
>     -   깔끔할 수 있지만 현실적이진 않음
>
> -   **사각 회전(oblique)**
>
>     -   요인 간 상관 허용 (연구에서는 보통 이게 더 현실적) 예: Promax, Oblimin
>
>     -   인문사회·행태 연구에서 *기본 선택*
