...
1\. **알파(α, Shape parameter)**: "형상 모수"

-   **직관적 의미**: 알파는 **사건의 횟수** 또는 **발생 건수**를 나타내며, 감마 분포의 모양을 결정. 알파가 사건의 평균적인 빈도 또는 반복되는 횟수를 나타낸다고 생각할 수 있음.

    -   예를 들어, 감마 분포가 시스템의 **고장 발생 시간**을 모델링할 때, 알파가 클수록 더 많은 고장이 발생하는 데 걸리는 시간을 나타냄

    -   **알파가 1일 때**: 감마 분포는 지수 분포와 같아지며, 이는 단일 이벤트(예: 단일 고장) 사이의 대기 시간을 의미.

    -   **알파가 클수록**: 더 많은 이벤트가 누적된 시간(예: 다섯 번의 고장 발생까지의 시간)을 의미하며, 분포의 **꼬리가 더 길어짐 (**오른쪽으로 더 퍼짐)

2\. **베타(β, Scale parameter)**: "척도 모수"

-   **직관적 의미**: 베타는 **시간의 단위** 또는 **이벤트가 발생하는 속도**. 사건이 발생하는 데 걸리는 **시간 척도**를 조절하는 역할.

    -   예를 들어, 베타는 이벤트 간의 **시간 간격**을 조절하여 사건 발생의 빈도나 속도를 결정. 베타는 주로 사건이 얼마나 **빨리 또는 느리게** 일어나는지를 나타냄.

    -   **베타가 클수록**: 사건이 느리게 발생하며, 시간 간격이 더 길어짐. 즉, 사건이 발생하기까지 걸리는 시간이 더 길다.

    -   **베타가 작을수록**: 사건이 더 자주 또는 빠르게 발생하여 시간 간격이 짧아짐.

    -   베타가 1일 때: 사건은 표준적인 속도로 발생.

    -   베타가 0.5라면, 사건이 발생하는 데 시간이 더 많이 걸린다는 뜻.

    -   베타가 2라면, 사건이 더 빨리 발생하며, 시간 간격이 짧아짐.
...
![](images/clipboard-3808897993.png)
